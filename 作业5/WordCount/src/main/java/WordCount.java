...
public class WordCount {
    ...
    public static class TokenizerMapper extends Mapper<Object, Text, Text, IntWritable> {   //继承泛型类Mapper
        ...
        private void parseSkipPunc(String fileName) {
            try {
        ...
        private void parseSkipWord(String fileName) {
            try {
                BufferedReader fis = new BufferedReader(new FileReader(fileName));
                String pattern = null;
                while ((pattern = fis.readLine()) != null) {
...
//    public static class SPOutPutFormat<K, V> extends MultipleOutputFormat<K, V> {
//        @Override
...
//                }
                ...
                job.getConfiguration().set("mapreduce.inputpath", f.getPath());
                FileOutputFormat.setOutputPath(job, new Path("midout7/"+f.getName()));
                job.waitForCompletion(true);
                Configuration conf2 = new Configuration();
                Job job2 = Job.getInstance(conf2, "word count2");   //实例化job
                job2.setJarByClass(WordCount.class);   //为了能够找到wordcount这个类
                ...
                FileInputFormat.addInputPath(job2, new Path("midout7/"+f.getName()));
                FileOutputFormat.setOutputPath(job2, new Path(otherArgs.get(1)+f.getName().split("\\.")[0]));   //添加输出路径
        ...
        finalJob.setJarByClass(WordCount.class);
//        finalJob.setOutputFormatClass(SPOutputFormat.class);
        ...
        String allPath = "";
        for (File f : fs) {
            allPath = allPath + "midout7/"+f.getName()+",";
...
//            FileOutputFormat.setOutputPath(finalJob, new Path(otherArgs.get(2)+"/TotalStatistics"));
        ...
        FileOutputFormat.setOutputPath(finalJob, new Path("midout7"+"/TotalStatistics"));
        ...
        finalJob2.setJarByClass(WordCount.class);   //为了能够找到wordcount这个类
        ...
        FileInputFormat.addInputPath(finalJob2, new Path("midout7"+"/TotalStatistics"));
        FileOutputFormat.setOutputPath(finalJob2, new Path(otherArgs.get(1)+"/TotalStatistics"));   //添加输出路径
